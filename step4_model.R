...
### 2.0 input genelist
geneLst <- intersect(intersect(MK.T,corMK.T),riskG) 
...
if(length(lasso.risk_factor)>7){
  stepwise.risk_factor <- c(0.40964,1.01761,0.07437,0.22458,-0.27886 ,-0.22451,0.21723,-0.08250,-0.72751)
  names(stepwise.risk_factor) <- c('RAMP1','HSPA4','ASCL1','FLRT3','MEIS1','RUNDC3B','GSTP1',
                                   'MEG3','CD200')
...
sur_df <- clin_info
sur_df$surtype <- 'high risk'
sur_df$surtype[sur_df$rs < cut_off]='low risk'
...
p <- ggsurvplot(fit, pval = TRUE,palette ='Set1',
                ...
                title='Survival curve of training set') ## days
p
ggsave(filename='FIG_scMK/survCurve_training.pdf',plot = last_plot(),width = 5,height = 4)
...
ggsave(filename=paste('FIG_scMK/survCurve_',validSet,'.pdf',sep=''),plot = last_plot(),width = 5,height = 4)
print(surv_pvalue(fit)$pval)
